...
void AdcDma::Stop()
{
	logdebug("Stopping AdcDma timer\n");
	stopTimer();
	logdebug("Stopping DMA\n");
	stopDma();
	logdebug("Stopping ADC\n");
	stopAdc();

	logdebug("Setting timer state stopped\n");
...
bool AdcDma::SetBuffers(int bufCount, int bufSize)
{
	if (m_captureState != CaptureStateStopped) {
		logwarning("Not in stopped state\n", __FUNCTION__);
		return false;
	}
	...
	if (bufCount > ADC_DMA_MAX_BUF_COUNT) {
		logwarning("bufCount (%d) is bigger than ADC_DMA_MAX_BUF_COUNT (%d)\n", bufCount, ADC_DMA_MAX_BUF_COUNT);
		return false;
	}

	if (bufCount * bufSize > ADC_DMA_MAX_MEM) {
		logwarning("total buf size (%d) is bigger than ADC_DMA_MAX_MEM (%d)\n", bufCount * bufSize, ADC_DMA_MAX_MEM);
		return false;
	}
	...
	if (!allocateBuffers(bufCount, bufSize, true)) {
		logwarning("failed in allocateBuffers !\n");
		return false;
	}
...
void AdcDma::updateAdcTimerChannel()
{
	...
	case 2:
		...
		break;
	default :
		logwarning("invalid timer channel %d !\n", m_timerChannel);
		return;
	}
...
bool AdcDma::SetTimerChannel(int timerChannel)
{
	if (timerChannel < 0 || timerChannel > ADC_DMA_MAX_TIMER_CHANNEL) {
		logwarning("invalid channel %d !\n", timerChannel);
		return false;
	}

	if (m_captureState != CaptureStateStopped) {
		logwarning("can't set timer channel while running !\n");
		return false;
	}
...
bool AdcDma::SetSampleRate(int sampleRate)
{
	if (sampleRate > ADC_DMA_MAX_SAMPLE_RATE) {
		logwarning("sampleRate is bigger than ADC_DMA_MAX_SAMPLE_RATE (%d)\n", ADC_DMA_MAX_SAMPLE_RATE);
		return false;
	}

	loginfo("%s, setting sample rate %d\n", __FUNCTION__, sampleRate);
...
bool AdcDma::SetChannelGain(int adcChannel, int gain)
{
	...
	case 4 :
		...
		break;
	default :
		logwarning("Invalid gain value %d", gain);
		return false;
	}
...
bool AdcDma::SetTrigger(int value, TriggerMode mode, int triggerChannel, int triggerTimeoutMs)
{
	...
	logdebug("Calculated m_triggerTimeoutMaxInts %d\n", m_triggerTimeoutMaxInts);
...
uint32_t AdcDma::SetBufferDuration(uint32_t durationMs)
{
	...
	logdebug("requested duration %d ms, actual %d ms\n", durationMs, actDurationMs);
...
bool AdcDma::SetTriggerPreBuffersCount(int triggerPreBuffersCount)
{
	if (triggerPreBuffersCount + 2 > m_bufCount) {
		logwarning("Too big value for triggerPreBuffersCount");
		return false;
	}

	if (triggerPreBuffersCount < 0) {
		logwarning("Negative value for triggerPreBuffersCount");
		return false;
	}
...
bool AdcDma::SetTriggerPreSamplesCount(int triggerPreSamplesCount)
{
	if (triggerPreSamplesCount < 0) {
		logwarning("Negative value for triggerPreSamplesCount\n");
		return false;
	}

	if (!SetTriggerPreBuffersCount(triggerPreSamplesCount / m_bufSize + 1)) {
		logwarning("Failed in  SetTriggerPreBuffersCount\n");
		return false;
	}
...
bool AdcDma::allocateBuffers(int bufCount, int bufSize, bool bForceAllocate)
{
	...
	for (int iBuf = 0; iBuf < m_bufCount; iBuf++) {
		...
		if (m_buffers[iBuf] == NULL) {
			logwarning("%s: failed allocating buffer %d, size %d !\n", __FUNCTION__, iBuf, m_bufSize);
			deleteBuffers();
			return false;
		}
		logdebug("m_buffers[%d]: 0x%08x\n", iBuf, m_buffers[iBuf]);
	}
...
bool AdcDma::configureTimer()
{
	...
	if (!bFound) {
		logwarning("%s: No prescaler found !\n", __FUNCTION__);
		return false;
	}
	...
	logdebug("AdcDma: Setting up AdcDma timer with parameters:\n");
	logdebug(" - Timer channel %d\n", m_timerChannel);
	logdebug(" - Sample rate %d\n", m_sampleRate);
	logdebug(" - Div %d\n", prescaler.div);
	logdebug(" - RC %d\n", RC);
...
bool AdcDma::advanceReadBuffer()
{
	...
	logdebug("Advanced readBuffer, m_readBufIndex is now %d\n", m_readBufIndex);
...
bool AdcDma::advanceWriteBuffer()
{
	...
	if ( ((m_writeBufIndex + 2) % m_bufCount) == m_readBufIndex) {
		logwarning("Would overwrite non read data (R %d, W %d)\n", m_readBufIndex, m_writeBufIndex);
		logwarning("BufCount! %d\n", m_bufCount);
		return false;
	}
	...
	logdebug("Setting buffer %d to RNPR, m_writeBufIndex %d\n", (m_writeBufIndex + 1)% m_bufCount, m_writeBufIndex);
...
void AdcDma::triggerUpdateState(TriggerEvent *event)
{
	...
	default :
		logerror("Invalid current state !\n");
		break;
	}
...
void AdcDma::HandleInterrupt()
{
	...
	if (isr & ADC_ISR_COMPE) {
		...
		logdebug("Got a ADC_ISR_COMPE interrupt\n");
	...
	if (isr & ADC_ISR_RXBUFF) {
		...
		logdebug("Got a ADC_ISR_RXBUFF interrupt\n");
		...
		default :
			logdebug("ADC_ISR_RX_BUFF is set, stopping acquisition !\n");
			break;
		}
	...
	if (isr & ADC_ISR_ENDRX) {
		...
		logdebug("ADC_ISR_ENDRX, IMR 0x%08x, trigger state %s\n", static_cast<uint32_t>(ADC->ADC_IMR), StrTriggerState[m_triggerState]);
		...
		if (m_rxHandler) {
			logdebug("Calling handler, bufIndex %d\n", bufIndex);
